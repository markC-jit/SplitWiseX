...
import { getBlockExplorerAddressLink } from "~~/utils/web3";
...
// Address Copy Icon Component
const AddressCopyIcon = ({ address, className }: { address: string; className?: string }) => {
  ...
  return (
    <button onClick={copyToClipboard} className={`${className} text-gray-500 hover:text-gray-700`}>
      {addressCopied ? <CheckIcon className="h-4 w-4 text-green-500" /> : <DocumentDuplicateIcon className="h-4 w-4" />}
    </button>
  );
};

// Address Link Wrapper Component
const AddressLinkWrapper = ({
  children,
  disableAddressLink,
  ...
  if (!checkSumAddress) {
    return (
      <div className="flex items-center">
        <div
          className="shrink-0 skeleton rounded-full bg-gray-300 animate-pulse"
          style={{
        ...
        <div className="flex flex-col space-y-1">
          {!onlyEnsOrAddress && (
            <div className={`ml-1.5 bg-gray-300 animate-pulse rounded-lg font-bold ${textSizeMap[ensSize]}`}>
              <span className="invisible">0x1234...56789</span>
            </div>
          )}
          <div className={`ml-1.5 bg-gray-300 animate-pulse rounded-lg ${textSizeMap[addressSize]}`}>
            <span className="invisible">0x1234...56789</span>
          </div>
        </div>
      </div>
